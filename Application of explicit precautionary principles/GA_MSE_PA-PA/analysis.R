...
### ------------------------------------------------------------------------ ###
### Care this is the input to see the history:
pol_ow <- readRDS("input/500_50/OM_1_hist/random/sol8c9a.rds")$stk
pol_rnd <- readRDS("input/500_50/OM_1_hist/random/sol8c9a.rds")$stk
pol_brp <- readRDS("input/sol8c9a.rds")
refpts <- pol_brp@refpts

df_hist <- rbind(
    cbind(
      as.data.frame(FLQuants(Fbar = fbar(pol_ow),
                    SSB = ssb(pol_ow))),
      fhist = "one-way"),
    cbind(
      as.data.frame(FLQuants(Fbar = fbar(pol_rnd),
                    SSB = ssb(pol_rnd))),
      fhist = "random")
)
df_hist <- df_hist %>%
