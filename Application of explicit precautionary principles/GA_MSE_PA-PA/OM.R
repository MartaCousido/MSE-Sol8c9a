...
### ------------------------------------------------------------------------ ###
### The same packages as OM1.R except mse.
...
vec1=colnames(stocks);ind=which(vec1=="lmax")
ind1=which(vec1=="h_Myers")
stocks=stocks[,-c(ind,ind1:length(vec1))]
colnames(stocks)=vec
...
brps <- readRDS("input/sol8c9a.rds")

### New code since Blim is required
brps=list(brps)
...
### create FLStocks
stocks_subset <- stocks$stock[1]#"pol"
...
res <- lapply(stocks_subset, function(stock) {
  stk <- readRDS(paste0("input/", n_iter, "_", yrs_proj, "/OM_1_hist/", fhist, 
                        "/", stock, ".rds"))$stk
  ssb(stk)[, ac(100)] / refpts(brps)["msy", "ssb"]
})
...
stks_mp <- foreach(stock = stocks_subset, .errorhandling = "pass", 
  ...
  oem <- FLoem(method = obs_generic,
               ...
               args = list(idx_dev = TRUE, ssb = FALSE,
                           ...
                           PA_Bmsy = c(refpts(brps)["msy", "ssb"]), 
                           PA_Fmsy = c(refpts(brps)["msy", "harvest"])))
  ctrl <- mpCtrl(list(
