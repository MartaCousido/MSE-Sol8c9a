...
args = c("use_MPI=FALSE", "n_workers=10", "n_blocks=10", "maxiter=1", "popSize=35", 
         "run=1", "stock_id=c(22)", "n_iter=500", "n_yrs=50", "multiplier=TRUE",
         "ga_search=FALSE")
# cl1 = NULL
### ------------------------------------------------------------------------ ###
### run MSE ####
...
names(stock) <- stock
input <- 
  readRDS(paste0("input/", n_iter, "_", n_yrs, "/OM_2_mp_input/", fhist, "/", "sol8c9a",
                        ".rds"))
